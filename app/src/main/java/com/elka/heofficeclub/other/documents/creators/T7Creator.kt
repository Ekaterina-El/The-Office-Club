...
class T7Creator(private val context: Context) : FormCreator(context) {
  override val assetName = "T_7.pdf"
  override val postfix: String = "T7/"

  override fun getFields(outputDoc: PdfDocument, docField: DocForm): List<PdfFormField> {
    val value = docField as T7
    val currentYear = getYearOfDate(Calendar.getInstance().time).toString()
    ...
    val list = mutableListOf(
      ...
      // for year
      DocField(
        Rectangle(530.88f, 388.32f, 48.36f, 13.2f),
        FieldType.DOC_NUMBER,
        currentYear,
        DocField.CENTER
      ),
